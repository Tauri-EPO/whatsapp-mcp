...
| **Media** | send files and voice notes (auto-converted to Opus), `read_media` returns the bytes to the agent itself — images as image content, everything else as text or base64, and PDF/DOCX/XLSX extracted to text on the server — so a client on another machine can actually read the attachment, recover expired attachments by asking the sender's phone, inventory of what is stored (size, duplicates across chats, cached or not), free space on request with a dry run first |
