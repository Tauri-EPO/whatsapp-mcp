...
``media_notes`` under the same keys ``transcribe_audio`` writes.
...
def find_pending(batch: int) -> list[Candidate]:
    ...
    for message_id, chat_jid, sha256 in rows:
        ...
        if cached is None:
            continue  # bytes never downloaded or already purged; nothing to read
        seen.add(sha256)
        out.append(
            Candidate(
...
def run_once(
    ...
    *,
    ...
    try:
        pending = find_pending(batch)
    except (sqlite3.Error, ToolError, OSError) as exc:
...
def run_forever(
    ...
    *,
    ...
    run: Callable[[int], BatchResult] = run_once,
) -> None:
...
def install_ingest_worker(env: Mapping[str, str] | None = None) -> threading.Thread | None:
    ...
    if load_config(env).backend is None:
        ...
        return None
    logger.info(
        "%s=1: transcribing up to %d inbound voice notes every %.0fs (whisper on this machine)",
        ENABLED_ENV,
        config.batch,
        config.interval_s,
    )
