...
SHA = {"AUD1": "a1" * 32, "AUD2": "a2" * 32, "OUT1": "0f" * 32, "BOB1": "b1" * 32}
...
def _add_audio(store, message_id: str, chat_jid: str, *, from_me: int = 0, cached: bool = True) -> None:
    ...
    filename = f"audio_20260905_09000{message_id[-1]}_{message_id}.ogg"
...
def test_worker_is_off_by_default_and_reads_its_knobs():
    ...
    on = transcribe_worker.load_ingest_config(
        {"TRANSCRIBE_ON_INGEST": "1", "TRANSCRIBE_ON_INGEST_INTERVAL_S": "60", "TRANSCRIBE_ON_INGEST_BATCH": "3"}
    )
    assert (on.enabled, on.interval_s, on.batch) == (True, 60.0, 3)
