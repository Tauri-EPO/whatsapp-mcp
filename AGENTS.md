...
└── .github/workflows/          # ci.yml, security.yml, publish.yml (main/sha images), release.yml (release PR, tags, latest images), release-cut.yml (daily merge of the release PR), build-push.yml (reusable)
```
...
`publish.yml` pushes the `main` and `sha-<7>` images to GHCR on every merge to `main`, and once more by dispatch after a scheduled cut (§4); `release.yml` (release-please) maintains the release PR and, when it merges, tags and pushes `vX.Y.Z` / `X.Y` / `latest` through the reusable `build-push.yml`; `release-cut.yml` merges that PR once a day. Dependabot auto-merge was removed; merge its PRs through the normal routine.
