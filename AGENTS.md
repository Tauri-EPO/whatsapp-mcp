...
| Docker Build | both images build with buildx (GHA cache); smoke: bridge starts and reports the FTS state, every module in `py-modules` imports inside the image; the bridge also cross-builds for `linux/arm64` |
