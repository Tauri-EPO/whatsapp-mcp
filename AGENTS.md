...
- **Dependencies move here first.** Dependabot stays on; its PRs are merged once CI is green. Bumps that change the shipped images (uv, gomod, docker, docker-compose) are titled `deps:` and cut a patch release; GitHub Actions bumps stay `chore(deps)`. Never pin a dependency just because upstream did (`mcp<2` and `cryptography<49` were both dropped).
...
`publish.yml` pushes the `main` and `sha-<7>` images to GHCR on every merge to `main`; `release.yml` (release-please) maintains the release PR and, when it merges, tags and pushes `vX.Y.Z` / `X.Y` / `latest` through the reusable `build-push.yml`; `release-cut.yml` merges that PR once a day. Dependabot auto-merge was removed; merge its PRs through the normal routine.
